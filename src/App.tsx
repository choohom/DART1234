    ...
    const damagedTableRows = damagedItems.map((item, index) => (
      new TableRow({
        children: [
          ...
          new TableCell({ children: [new Paragraph({ text: item.totalPrice.toLocaleString(), alignment: AlignmentType.RIGHT })] }),
        ],
      })
    ));
    ...
    const doc = new Document({
      ...
      sections: [{
        ...
        children: [
          ...
          new Paragraph({
            children: [new TextRun({ text: "เรื่อง   การประเมินค่าเสียหายที่เกิดขึ้นกับระบบจำหน่าย", bold: true })],
            spacing: { before: 200 },
          }),
          ...
          new Paragraph({
            children: [
              new TextRun({ text: `11. รายการอุปกรณ์ที่ได้รับความเสียหาย ${totalItems} รายการ คิดเป็นค่าเสียหาย จำนวนเงินทั้งสิ้น ${totalAmount.toLocaleString()} บาท โดยมีรายละเอียดดังนี้` }),
            ],
            spacing: { before: 400 },
          ...
          new Paragraph({
            text: `11.1 รื้อถอน - ติดตั้งใหม่ ${damagedCount} รายการ เป็นจำนวนเงินทั้งสิ้น ${damagedTotal.toLocaleString()} บาท`,
            indent: { left: 1701 },
          ...
          new Paragraph({
            text: `11.2 แผนกซ่อมแซม ${reusableCount} รายการ เป็นจำนวนเงินทั้งสิ้น ${reusableTotal.toLocaleString()} บาท`,
            indent: { left: 1701 },
          ...
          new Paragraph({
            text: `จึงเรียนมาเพื่อพิจารณาอนุมัติให้ดำเนินการเบิกอุปกรณ์ไปซ่อมแซมตามรายการดังกล่าว พร้อมทั้งเป็นการเรียกเก็บเงินค่าเสียหายจากผู้กระทำละเมิด เป็นจำนวนเงินทั้งสิ้น ${totalAmount.toLocaleString()} บาท`,
            indent: { firstLine: 1417 }, // 2.5 cm
            spacing: { before: 600 },
            alignment: AlignmentType.THAI_DISTRIBUTE,
          }),
    ...
    <div className="min-h-screen bg-slate-50 pb-20">
        ...
        <div className="max-w-6xl mx-auto px-4 h-20 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            <div className="text-right hidden sm:block">
              ...
              <p className="text-xl font-black text-blue-600">฿{totalAmount.toLocaleString()}</p>
            </div>
          </div>
        </div>
      </header>
                    ...
                    <div className="p-4 bg-blue-50 border border-blue-100 rounded-xl flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-sm text-blue-700">
                          ...
                          ชำรุด: ฿{selectedMaterial.priceDamaged.toLocaleString()} | 
                          นำกลับมาใช้ใหม่: ฿{selectedMaterial.priceReusable.toLocaleString()}
                        </p>
                      </div>
                      <button 
                        onClick={() => {
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-1 gap-3">
                    ...
                    >
                      ...
                      <span className="text-xs font-bold bg-red-100 text-red-600 px-2 py-1 rounded">฿{((selectedMaterial?.priceDamaged || 0) * quantity).toLocaleString()}</span>
                    </button>
                    <button
                      disabled={!selectedMaterial}
                    ...
                    >
                      ...
                      <span className="text-xs font-bold bg-green-100 text-green-600 px-2 py-1 rounded">฿{((selectedMaterial?.priceReusable || 0) * quantity).toLocaleString()}</span>
                    </button>
                  </div>
                </div>
              </div>
          ...
          <div className="space-y-6">
            <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 flex flex-col h-full max-h-[700px]">
                    ...
                    >
                      ...
                      <div className="flex justify-between items-end">
                        <p className="text-xs text-slate-500">
                          {item.quantity} {item.material.unit} x ฿{(item.status === 'damaged' ? item.material.priceDamaged : item.material.priceReusable).toLocaleString()}
                        </p>
                        <p className="font-bold text-blue-600">฿{item.totalPrice.toLocaleString()}</p>
                      </div>
                    </motion.div>
                  ))
                )}
              ...
              <div className="mt-6 pt-6 border-t border-slate-100 space-y-4">
                ...
                <div className="space-y-4">
                  <div className="flex items-center gap-2 text-blue-600">
                    <h2 className="font-bold">STEP 4 : ออกรายงาน</h2>
                  </div>
                  <button 
                    disabled={items.length === 0}
                    onClick={exportWord}
                    className="w-full flex items-center justify-center gap-2 py-4 bg-[#EF0107] text-white font-bold rounded-xl hover:bg-[#DB0007] transition-all disabled:opacity-50 disabled:cursor-not-allowed shadow-lg shadow-red-100"
                  >
                    <Download className="w-5 h-5" />
                    ออกรายการประเมิน (Word)
                  </button>
                </div>
              </div>
            </div>
      ...
      <div className="sm:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-slate-200 p-4 flex items-center justify-between z-20">
        ...
        <button 
          disabled={items.length === 0}
          onClick={exportWord}
          className="bg-[#EF0107] text-white px-6 py-3 rounded-xl font-bold text-sm flex items-center gap-2 hover:bg-[#DB0007] transition-colors"
        >
